            ...
            <div class="row align-items-center">
                <div class="col-md-2 text-center">
                    @if ($employee->picture && $employee->picture !== 'default.png')
                        <img src="{{ asset('uploads/employees/' . $employee->picture) }}" class="employee-avatar"
                ...
                <div class="col-md-6">
                    ...
                    <p class="mb-1 h5 font-weight-light">
                        {{ $employee->rel_to_designation->designation_name ?? 'No Designation' }}</p>
                    <p class="mb-0 text-light">{{ $employee->rel_to_departmet->department_name }} •
                        {{ $employee->rel_to_companies->company }}</p>
                </div>
        ...
        <div class="nav-tabs-custom">
            <div class="d-flex justify-content-between align-items-center">
                <ul class="nav nav-tabs border-0" id="employeeTab" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" href="{{ route('employee_assets', ['emp_id' => $employee->emp_id]) }}">
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div class="btn-group">
                        ...
                        <a href="{{ route('pdf_history', ['emp_id' => $employee->emp_id, 'search' => request('search')]) }}"
                            ...
                            @foreach ($issues as $issue)
                                <tr>
                                    <td class="text-center">
                                        <input type="checkbox" class="asset-checkbox" value="{{ $issue->asset_tag }}"
                                            data-issue-date="{{ $issue->issue_date }}"
                                            data-return-date="{{ $issue->return_date }}"
                                            data-asset-type="{{ $issue->asset_type }}" data-model="{{ $issue->model }}"
                                            data-description="{{ $issue->description }}">
                                    </td>
                                    <td>
                                        <a href="{{ route('store', ['asset_tag' => $issue->asset_tag]) }}">
                                            <span class="fw-medium">{{ $issue->asset_tag }}</span>
                                        </a>
                                    <td>
                                        <div class="d-flex flex-column">
                                            <span class="fw-medium">{{ $issue->asset_type }}</span>
                                            <small class="text-muted">{{ $issue->model }}</small>
                                        </div>
                                    </td>
                                    <td>
                                        <span>{{ $issue->description ?: 'N/A' }}</span>
                                        @if ($issue->note)
                                            <small class="d-block text-muted">Note: {{ $issue->note }}</small>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="d-flex flex-column">
                                    ...
                                    <td>
                                        @if ($issue->return_date)
                                            <span>{{ \Carbon\Carbon::parse($issue->return_date)->format('M d, Y') }}</span>
                                        @else
            ...
            <!--uploads modal-->
            <div class="modal fade upload-modal" id="uploadsModal" tabindex="-1" aria-labelledby="uploadsModalLabel"
                aria-hidden="true">
                    ...
                    <div class="modal-content border-0">
                        ...
                        <div class="modal-body">
                            <form id="fileUploadForm"
                                action="{{ route('employee.storeOtherFile', ['id' => $employee->id]) }}" method="POST"
                                ...
                                <div class="alert alert-info">
                                    ...
                                    <small class="d-block mt-1">PDF, DOC, DOCX, XLS, XLSX, PNG, JPG, JPEG, GIF, ZIP, and
                                        more</small>
                                </div>
                            </form>
                        </div>
                            ...
                            <div class="modal-body">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" id="return_emp_name" name="emp_name" class="form-control"
                                            readonly>
                                    </div>
                                    <div class="mb-3">
            ...
            setTimeout(function() {
                ...
                assetCheckboxes.forEach(function(checkbox) {
                    checkbox.addEventListener('change', function() {
                        console.log('Individual checkbox changed:', this.value, this
                            .checked);
                ...
                function updateSelectAllCheckbox() {
                    ...
                    console.log('Checked boxes count:', checkedBoxes.length, 'Total:', assetCheckboxes
                        .length);
