...
@section('content')
    ...
    <div class="container">
        <!-- Page Header -->
        <div class="card mb-4"
            style=" background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);">
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 text-white">
                        ...
                        @if ($pendingRequests->count() > 0)
                            <span
                                class="badge bg-light text-dark ms-2 notification-pulse">{{ $pendingRequests->count() }}</span>
                        @endif
                    </h5>
                    ...
                    <div class="col-md-6 col-lg-4 mb-4">
                        <div class="card request-card h-100">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-2">
                                    <strong>Asset SL No:</strong> {{ $request->asset_sl_no }}
                                </div>
                                @if ($request->model)
                                    <div class="mb-2">
                                    ...
                                    <form method="POST" action="{{ route('approve_transfer_request', $request->id) }}">
                                        ...
                                        <div class="modal-body">
                                            <div class="alert alert-info">
                                                <strong>Asset:</strong> {{ $request->asset_tag }}
                                                ({{ $request->asset_type }})
                                                <br>
                                                <strong>Transfer:</strong> {{ $request->from_company }} →
                                                {{ $request->to_company }}<br>
                                                <strong>Asset SL No:</strong> {{ $request->asset_sl_no }}
                                            </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="item_status" class="form-control" required>
                                                    <option value="transfer"
                                                        {{ $request->item_status === 'transfer' ? 'selected' : '' }}>
                                                        Permanent Transfer - Ownership changes to receiving company
                                                    </option>
                                                    <option value="borrowed"
                                                        {{ $request->item_status === 'borrowed' ? 'selected' : '' }}>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <textarea name="approval_notes" class="form-control" rows="3" placeholder="Optional notes about the approval..."></textarea>
                                            </div>
                                        </div>
        ...
        @else
            <div class="card">
                <div class="card-body text-center py-5">
                    ...
                    <p class="text-muted">All transfer requests have been processed or there are no new requests waiting
                        for approval.</p>
